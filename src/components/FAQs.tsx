...
const AccordionItem = ({ item, isOpen, onClick }: { item: any; isOpen: boolean; onClick: () => void; }) => (
    <div className="border-b border-slate-200">
        <button onClick={onClick} className="w-full flex justify-between items-center text-left gap-4 py-6">
            <h3 className="text-lg font-semibold text-slate-800">{item.question}</h3>
            <div className="flex-shrink-0">
                {isOpen ? <Minus className="w-6 h-6 text-slate-500" /> : <Plus className="w-6 h-6 text-slate-500" />}
            </div>
        </button>
        <AnimatePresence>
            {isOpen && (
